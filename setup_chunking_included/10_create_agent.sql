...
CREATE OR REPLACE AGENT AGENTS.MCC_PRODUCT_CHATBOT_CHUNKED
  COMMENT = 'Chatbot for MCC (Micro Commercial Components) product documentation and catalog data (chunked version)'
  PROFILE = '{"display_name": "MCC Product Assistant (Chunked)", "avatar": "📦"}'
  FROM SPECIFICATION $$
  {
    "models": {
    ...
    "tool_resources": {
      "search_mcc_products": {
        "search_service": "PRODUCT_AGENT.DATA.MCC_PRODUCT_SEARCH",
        "max_results": 5,
        "columns": ["chunk_text", "product_name", "document_type", "source_file"]
      },
      "query_product_catalog": {
        "semantic_view": "PRODUCT_AGENT.DATA.MCC_PRODUCT_CATALOG",
